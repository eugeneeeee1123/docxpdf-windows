...
- 批量转换默认使用最多两个独立 Word 会话并行处理，保持列表顺序与合并顺序
- 支持桌面端横向双栏工作区（左侧文件队列、右侧输出设置），窄窗口自动回退并压缩控件，以及浅色/深色主题切换并记住选择
- 支持中文、空格和较长文件名路径
...
当前测试覆盖中英文界面、主题切换与横向双栏响应式布局、翻译完整性、Word COM 参数与清理、原子输出、错误与取消、并行批处理、原始像素恢复、合并顺序，以及合并时图片流不被重新编码。

批量转换默认开启两个并行 Word 会话；如果某台电脑上的 Word 或内存资源较紧张，可以在启动前设置 `DOCXPDF_WORD_WORKERS=1`，也可以使用 `2` 到 `4` 调整并行度。
